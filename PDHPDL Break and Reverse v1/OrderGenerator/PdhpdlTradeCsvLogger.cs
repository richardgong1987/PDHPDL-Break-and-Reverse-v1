...
public class PdhpdlTradeCsvLogger {
    ...
    public void Append(PdhpdlTradeCsvRecord record) {
        ...
        string line = string.Join(",", Escape(nextId.ToString(CultureInfo.InvariantCulture)), Escape(record.Side), Escape(record.KeyLevel),
            Escape(record.Signal), Escape(record.CloseEntryResult), Escape(record.Pullback25Result), Escape(record.Pullback382Result),
    ...
    private static string BuildHeader() {
        return string.Join(",", "编号", "多空", "关键位", "信号", "收线入场", "回撤25入场", "回撤38.2入场", "回撤50入场", "备注", "交易品种", "时间周期", "入场时间", "入场价格",
            "止损价格", "第一止盈价格", "第二止盈价格", "风险价格距离", "下单数量");
    }
